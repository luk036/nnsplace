...
class TestNnsPlacer:
    ...
    def test_nnsplacer_init(self, mock_netlist, mock_nnsconfig):
        ...
        assert placer.count == [[0 for _ in range(mock_nnsconfig.grid[0] + 2)], [0 for _ in range(mock_nnsconfig.grid[1] + 2)]]
    ...
    def test_calc_worst_wirelength_v(self, mock_netlist, mock_nnsconfig):
        ...
        # (0,2): x_diff = 2, y_diff = 2. cost = 2*1 + 2*2 = 6
        # Worst for v=0 should be 6
        assert placer.calc_worst_wirelength_v(0, place) == 6
    ...
    def test_init_placement(self, mock_netlist, mock_nnsconfig, mocker):
        ...
        mocker.patch('nnsplace.placement.shuffle', side_effect=lambda x: None) # No actual shuffling
